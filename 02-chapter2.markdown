...
### 복제(Duplication)
마이크로서비스의 목적은 비용을 줄이는 것이 아닙니다. 시간이 지남에 따라 이러한 현상이 발생할 수 있지만, 통합 및 표준화를 통한 비용절감은 마이크로서비스의 이점이 아닙니다. 기존 기업에서는 CIO가 하나의 데이터베이스, 하나의 애플리케이션 서버 등을 구입하여 모든 사용자에게 이를 사용하도록 강요했습니다. 많은 기업들이 한 단계 더 나아가서 하나의 대규모 DB, 하나의 대규모 애플리케이션 서버 클러스터 등을 구축할 것입니다. 비용을 절감하는 것이 목표라면 이는 큰 의미가 있습니다. 하지만 그것은 속도를 크게 저하시키는 결합을 도입하기 때문에 마이크로서비스와는 작동하지 않습니다.
...
제품 자체 외에도, 복제의 또 다른 큰 형태는 사례 자체입니다. 단일 애플리케이션 또는 마이크로서비스에서 사용하는 하나의 거대한 데이터베이스를 만드는 것이 기술적으로 더 효율적입니다. 하지만 다시 말하지만, 그것은 속도를 해치는 결합을 만들어냅니다. DBA팀이 패치 적용을 위해 데이터베이스를 해체해야 하는 경우에는 누구나 계획을 세워야 합니다. 마이크로서비스는 기술적으로 가장 효율적이지 않습니다. 각 마이크로서비스 팀은 항상 자체 스택을 선택하고, 때로는 조달하고, 실행합니다. 하지만 마이크로서비스의 핵심은 속도이다.
