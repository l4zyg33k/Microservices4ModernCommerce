...
1. 각 프로그램이 한 가지 일을 잘하도록 만드십시오. 새로운 일을 하기 위해서는, 오래 된 프로그램에 새로운 기능을 추가함으로써 복잡하게 하기보다는 새롭게 만드세요.
2. 모든 프로그램의 출력이 아직 알려지지 않은 프로그램의 입력이 될 수 있습니다. 불필요한 정보로 출력을 혼란스럽게 하지 마십시오. 열 또는 이진 입력 형식을 사용하지 마십시오. 대화식 입력을 고집하지 마십시오.
3. 몇주 이내에 시도할 수 있도록 소프트웨어를 설계하고 구축하는 것이 좋습니다. 어색한 부분들을 버리고 다시 만드는 것을 주저하지 마십시오.
...
![그림 2-2. 마이크로서비스는 시간에 지남에 따라 복잡성이 감소](https://raw.githubusercontent.com/l4zyg33k/Microservices4ModernCommerce/master/figures/fig2-2.png)

_그림 2-2. 마이크로서비스는 시간에 지남에 따라 복잡성이 감소_
...
우리가 이미 살펴본 것 처럼, 조직의 구조가 소프트웨어 생성을 어떻게 해야하는지 정합니다. 계층을 중심으로 하는 중앙화 된 조직은 계층 된 단일 어플리케이션 생성을 지향합니다. 단순한 변경일 지라도 계층 전체에 광범위한 조정이 필요하기 때문에 시간이 더 소요됩니다. 마이크로서비스를 만들기를 원하는 조직을 위한 더 나은 구조는 제품을 재구성 하는 것 입니다. 운영의 VP보다 제품카탈로그에 관련된 마이크로서비스 롤업된 제품 카탈로그의 VP가 있어야 합니다. 이것은 생각의 기초적인 전환이지만, 소프트웨어보다 궁극적으로 더 생산적일 것입니다. 우리가 짧게 다룰 DevOps는 개발과 운영 사이의 구조적인 경계를 허무는 한걸음입니다. 이러한 전환이 잘 진행된다면, 팀경계가 모호해지는 것이 그렇게 이상하지 않을 것입니다.

문화적 시각에서, 마이크로서비스는 IT는 줄여야할 비용이라고 간단하게 생각하는 관념을 버릴 것을 요구합니다. 비록 중앙화된 팀이 각 계층(개발, 운영, 인프라 등등)을 다루는 것은 효율적일지라도, 그러한 효울은 개발속도가 크게 느려지는 결합 비용을 야기시킵니다. 조직의 문화는 비즈니스에 대한 개발속도와 반응성에 가치를 둘 필요가 있습니다.

구조와 문화를 넘어, 조직은 Agile 소프트웨어 개발, DevOps 와 클라우드에 대한 강한 이해를 가질 필요가 있습니다. 대부분의 마이크로서비스 팀은 각각의 마이크로 서비스를 관리하기 위해 Agile의 몇몇 형탸를 사용합니다. 폭포수 모델에서 Agile로 변화하는 것은 어렵습니다. DevOps는 개발팀과 운영팀이 협력적으로 혹은 교대로 어플리케이션의 라이프사이클을 보조합니다. 각 마이크로서비스 팀은 모든 개발과 운영을 책임지고 있습니다. 따라서 DevOps는 운영모델입니다. 마지막으로, 클라우드는 마이크로서비스를 위한 요구사항입니다. 대부분의 마이크로서비스는 클라우드에 구축됩니다. 하지만 더욱 중요한 것은 클라우드가 그 자체의 특성에 의해 분산되고 분산컴퓨팅에 대한 전문지식을 갖추면 모든 것이 분산되어 있기 때문에 마이크로서비스로 전환하기가 더 쉽다는 것입니다.

### 복제
마이크로서비스의 목적은 비용을 줄이는 것이 아닙니다. 시간이 지남에 따라 이러한 현상이 발생할 수 있지만, 통합 및 표준화를 통한 비용절감은 마이크로서비스의 이점이 아닙니다. 기존 기업에서는 CIO가 하나의 데이터베이스, 하나의 어플리케이션 서버 등을 구입하여 모든 사용자에게 이를 사용하도록 강요했습니다. 많은 기업들이 한 단계 더 나아가서 하나의 대규모 데이터베이스, 하나의 대규모 어플리케이션 서버 클러스터 등을 구축할 것입니다. 비용을 절감하는 것이 목표라면 이는 큰 의미가 있습니다. 하지만 그것은 속도를 크게 저하시키는 결합을 도입하기 때문에 마이크로서비스와는 작동하지 않습니다.

각 마이크로서비스 팀은 독립적이며 각 계층에 가장 적합한 제품을 선택할수 있어야 합니다. 앞서 논의한 바와 같이, 각 팀에는 선택할 수 있는 지원 제품 메뉴가 주어져야 합니다. 예를들어, 프로그래밍언어는 Java, Scala 혹은 Node.js 가 될 수 있습니다. 팀은 이러한 모든 옵션에 대한 공식 제품 지원을 구입할 수 있습니다. 하나의 제품보다는 잠재적으로 현재 얼마 되지 않습니다. 비용에 초점을 맞춘 많은 관리자들은 "Java만 사용하세요"라고 말할 것이지만, 그것은 너무 제약적일 수 있습니다. 언어와 런타임에는 각각 고유한 차이가 있습니다.

제품 자체 외에도, 복제의 또 다른 큰 형태는 사례 자체입니다. 단일 어플리케이션 또는 마이크로서비스에서 사용하는 하나의 거대한 데이터베이스를 만드는 것이 기술적으로 더 효율적입니다. 하지만 다시 말하지만, 그것은 속도를 해치는 결합을 만들어냅니다. DBA팀이 패치 적용을 위해 데이터베이스를 해체해야 하는 경우에는 누구나 계획을 세워야 합니다. 마이크로서비스는 기술적으로 가장 효율적이지 않습니다. 각 마이크로서비스 팀은 항상 자체 스택을 선택하고, 때로는 조달하고, 실행합니다. 하지만 마이크로서비스의 핵심은 속도입니다.

### 궁극적인 일관성
마이크로서비스를 도입한 회사들이 갖고있는 가장 큰 문제들 중 하나는 모든 데이터가 일관성을 갖고 있지 않다는 것입니다. 제품은 20개의 다른 마이크로서비스로 존재할 수 있으며, 각 마이크로서비스에는 다른 시점의 제품 복사본을 갖고있습니다. 기업은 항상 최신 상태로 유지되는 제품의 복사본인 모든 상거래 데이터네 대해 단일 데이터베이스를 사용하는 데 익숙합니다. 그것은 개발속도에 방해가 되는 결합을 도입하기 때문에 마이크로서비스 모델이 아닙니다.

커머스플랫폼 안에서 과거에는 기업들이 데이터가 매우 일관될 것으로 기대해왔습니다. 하지만 저사적으로 데이터는 일관되었던 적이 없습니다. 예를들어, CRM, ERP, 커머스 어플리케이션은 각각 고객을 나타내는 고유한 표현을 가지고 있으며 어플리케이션 간에 데이터 업데이트가 비동기적으로 일어납니다.

마이크로서비스 기반의 커머스시스템에는 항상 최신 데이터를 가진 마이크로서비스가 적어도 하나 있습니다. 예를들어, 제품 마이크로서비스는 모든 제품 데이터를 갖고있습니다. 그러나 제품카탈로그와 검색 마이크로서비스는 제품 데이터의 캐시된 복사본을 갖고있을 것입니다. 위키나 다른 내부 공간에 어떤 마이크로서비스가 어떤 데이터를 소유하고 있는지 문서화하는 것을 고려해보세요.

### 테스트
마이크로서비스는 테스트를 쉬운 동시에 더욱 어려워집니다. 개발자의 컴퓨터에서 시작해 생산에 착수해보세요.
개발자는 가능한 적은 변수로 코드가 격리되어 작동하는지 확인하기 위해 단위테스트를 시행할 필요가 있습니다. 이 테스트는 코드베이스가 훨씬 작기 때문에 마이크로서비스를 이용하면 훨씬 쉽습니다. 전체 마이크로서비스는 격리된채로 많은 비판을 받지 않고 격리된채로 로컬에서 작동할 수 있습니다. 개발자들이 실제 소유자이기 때문에 그들은 더나은 코드를 작성하고 단위테스트 적용범위를 늘리기 위해 더 강력한 인센티브를 받습니다.

단위테스트와 마찬가지로, 구성요서 테스트도 꽤 쉽습니다. 각 마이크로서비스는 환경에서 스스로 작동하기 쉽습니다. 종종 하나의 API만 노출됩니다. 이벤트도 비판당할 수 있고, 어떻게 이벤트를 처리하는 지 보기 위해 마이크로서비스로 전달될 수도 있습니다.

원격환경에서의 통합테스트를 수행하는 것은 마이크로서비스로 인하여 더욱 어려워집니다. 이는 더이상 몇가지 단일 어플리케이션(수십개, 수백개 또는 수천개의 작은 마이크로서비스)과 통합되는 하나의 어플리케이션이 아닙니다. 마이크로서비스의 기능을 제대로 테스트하려면 종종 마이크로서비스를 사용하는 이벤트를 만들기 위해 다른 마이크로서비스를 호출하는 테스트 스크립트를 작성해야합니다. 예를들어, 주문상태 마이크로서비스를 테스트 하는 경우 먼저 주문 마이크로서비스를 사용하여 주문을 만들어야합니다. 이러한 복잡성은 내부의 복잡성과 외부의 복잡성을 교환하는 마이크로서비스의 필수적 교환입니다. 통합테스트는 어렵지만, 마이크로서비스로 얻는 빨라진 개발 속도가 이러한 추가적인 작업을 할 가치가 있게 만듭니다.

### 모니터링
테스트와 마찬가지로 모니터링도 쉬운 동시에 더욱 어려워집니다. 각 마이크로서비스의 상태는 평가하기 매우 쉽습니다. 평가하기가 더 까다로운 것은 전체 시스템의 건강입니다. 주문 상태 마이크로서비스를 사용할 수 없고 응용 프로그램이 실패를 적절하게 처리할 수 있는 경우에는 문제가 없습니다. 건강한 것과 건강하지 않은 것의 경계는 어디에 있을까요? 어느 시점에서 교통을 차단하고 문제를 해결해야 하기 때문에 이 선이 문제가 됩니다. 반면에, 획일화된 어플리케이션은 모니터링하기 쉽습니다. 획일화된 어플리케이션은 일반적으로 작동하거나 작동하지 않습니다. 마이크로서비스가 야기하는 또 다른 문제는 end-to-end 트랜잭션 추적입니다. 수십개, 수백개 또는 수천개의 마이크로서비스 중에서 문제를 일으키는 것은 무엇일까요? 마이크로서비스를 식별하고, 후에 종속성을 추적해야 합니다. 아마도 마이크로서비스 업스트림이 게시 이벤트를 중지했을 수도 있습니다. 금방 복잡해집니다. 일부 기술과 프로세스를 구현할 수 있다면 관리가 용이해 집니다. 그러나 이것은 대부분의 기업들이 마이크로서비스의 가장 큰 문제라고 말하는 것입니다. 단일 어플리케이션은 느리지만 문제의 범위는 적습니다.

## 마이크로서비스를 점진적으로 도입하는 방법

마이크로서비스를 채택하는 데는 본질적으로 세가지 다른 접근법이 있습니다. 그것들을 하나씩 살펴보도록 합시다.
...
작은 애플리케이션을 구축하는 경우 또는 복잡성으로 인해 초기의 마이크로서비스 오버 헤드가 정당화될지 여부를 모르는 경우에는 이 방식을 사용하는 것이 바람직하지 않습니다. 마이크로서비스는 수백 또는 수천의 개발자들과 수백만개의 코드 라인을 작성할 때 가장 적합합니다. 너무 일찍 최적화하지 마십시오.
...
예를 들어, 새로운 비즈니스 요구 사항으로 인해 단일 프로그램 가격을 철저히 검토해야 한다고 가정해 보겠습니다. 이를 단일 프로그램에서 수행하는 대신 마이크로서비스의 가격 책정을 다시 고려하는 기회로 활용하십시오. 단일 프로그램은 가격을 책정하기 위해 마이크로 서비스를 다시 호출합니다.
시간이 지남에 따라, 점점 더 많은 기능들이 단일 프로그램에서 벗어나 결국에는 사라집니다.

이 모델은 오늘날 대부분의 상거래 플랫폼의 표준 운영 절차입니다. 지불, 세금 계산, 배송 또는 평점 및 리뷰를 조직 내부에서는 거의 하지 않습니다. 모두 외부 공급 업체에 위탁되어 있습니다. 기능을 별도의 마이크로서비스로 구분하는 것은 동일한 모델입니다. 단, 호출하려는 API 뒤의 팀이 회사의 일부라는 점만 다릅니다.

### 단일성 해체 (Decompose the Monolith)
이는 단일 애플리케이션이 어떻게 작성되었는지에 따라 가장 어렵거나(또는 잠재적으로 가장 쉬운)접근 방식입니다. 애플리케이션이 충분히 모듈화된 경우 모듈을 꺼내어 독립형 마이크로서비스로 배포할 수 있습니다(그림 2-16).
...
여기서 어려운 점은 마이크로서비스가 모두 동일한 데이터베이스에 코드로 작성된다는 것입니다. 단일 애플리케이션에 하나의 데이터베이스만 있을 수 있기 때문입니다.
